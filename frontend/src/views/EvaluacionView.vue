...
// 2. BASE DE DATOS LOCAL: 5 Criterios Reales del Área 2
const area = ref({
...
// 3. MOTOR DE CÁLCULO
const mediaArea = computed(() => {
  let totalNotas = 0
  let cantidadEvaluados = 0

  area.value.competencias.forEach(comp => {
...
// 4. TRADUCTOR DE NOTA A TEXTO (El nuevo código)
const infoMedia = computed(() => {
  const media = mediaArea.value

  if (media === 0) {
    return { titulo: 'Sin evaluar', color: '#f3f4f6', borde: '#d1d5db', textoColor: '#6b7280' }
  }

  // Lógica de redondeo
  if (media < 1.5) return escala[0] // Poco Adecuado
  if (media < 2.5) return escala[1] // Adecuado
  if (media < 3.5) return escala[2] // Muy Adecuado
  return escala[3]                  // Excelente
})

// Función para guardar la nota al hacer clic en una columna
...
<template>
  <div class="contenedor-evaluacion">
    <header class="cabecera">
      ...
      <div class="caja-media">
        <div class="media-numerica">
          Media del Área: <strong>{{ mediaArea > 0 ? mediaArea.toFixed(2) : '0.00' }}</strong> / 4.00
        </div>

        <div
          class="etiqueta-resultado"
          :style="{
            backgroundColor: infoMedia.color,
            borderColor: infoMedia.borde,
            color: infoMedia.textoColor
          }"
        >
          {{ infoMedia.titulo }}
        </div>
    ...
    <div class="bloque-area">
      <h2 class="titulo-area">{{ area.nombre }}</h2>

      <div v-for="competencia in area.competencias" :key="competencia.id" class="bloque-competencia">
        ...
        <div v-for="criterio in competencia.criterios" :key="criterio.id" class="bloque-criterio">
          ...
          <div class="grid-rubrica">
            <div
              ...
              :style="{
                borderColor: criterio.nota === nivel.valor ? nivel.borde : 'transparent',
                backgroundColor: criterio.nota === nivel.valor ? nivel.color : '#f9fafb'
              }"
    ...
    <div class="panel-guardar">
      <button class="btn-guardar-final">Guardar Evaluación</button>
    </div>
  </div>
</template>
...
.cabecera { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; border-bottom: 2px solid #e5e7eb; padding-bottom: 15px; }
...
/* NUEVOS ESTILOS PARA LA CAJA DE MEDIA */
...
.panel-guardar { text-align: right; margin-top: 40px; padding-top: 20px; border-top: 2px solid #e5e7eb; }
